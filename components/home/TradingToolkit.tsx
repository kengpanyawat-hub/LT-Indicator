      ...
      <div className="container mx-auto px-4">
        <div className="space-y-12">
          {toolkitData.map((item, index) => (
            <div
              key={index}
              className={`bg-[#121212] border border-Red-500 rounded-3xl p-8 md:p-12 shadow-[0_0_30px_rgba(255,15,15,0.3)] grid grid-cols-1 md:grid-cols-2 gap-8 items-center ${item.reversed ? 'md:grid-flow-col-dense' : ''}`}>
              <div className={`order-2 ${item.reversed ? 'md:order-1' : ''}`}>
                <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
                  {item.title.split(' ').map((word, i) => (
                    <span key={i} className={['Ultimate', 'Winning', 'Smart'].includes(word) ? 'text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-red-500' : ''}>
                      {word}{' '}
                    </span>
                  ))}
                </h2>
